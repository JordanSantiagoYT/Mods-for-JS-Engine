characterPath = 'characters/rhm' -- Picture File name of spreadsheet (must have .xml)
characterTag = 'THEDUDEWITHAROBOGUN' -- Set this to what the sprite should be named.

noteNames = {
    'RHM Note', -- Change this to whatever your note is named in path "custom_notetypes".
...
objectOrder = 2 -- Normally, values over 7 go over GF (7), Dad (8) and BF (9). (Default: 9)
...
function onCreatePost()
    setProperty('THEDUDEWITHAROBOGUN.alpha', 0);
end


function onSongStart()
doTweenAlpha('graasdfasdy', 'THEDUDEWITHAROBOGUN', 1, 1, 'linear')
end
function onStepHit()
if curStep == 3391 then
doTweenAlpha('grfasda', 'THEDUDEWITHAROBOGUN', 0, 0.1, 'linear')
end
if curStep == 1431 then
doTweenAlpha('gfdr', 'THEDUDEWITHAROBOGUN', 0, 1, 'linear')
end
if curStep == 1685 then
doTweenAlpha('gss', 'THEDUDEWITHAROBOGUN', 1, 0.1, 'linear')
end
end
...
function opponentNoteHit(membersIndex, noteData, noteType, isSustainNote) 
    ...
    if noteType == noteNames[i] and not isDanceIdle then
...
function goodNoteHit(membersIndex, noteData, noteType, isSustainNote) 
    ...
    elseif noteType == noteNames[i] and not isDanceIdle then
        --playAnim(characterName, animNames[noteData + 2], true); -- uhhh is this needed?
        animSuffix = ''
