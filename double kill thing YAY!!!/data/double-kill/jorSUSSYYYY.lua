characterPath = 'characters/jorsawsee' -- Picture File name of spreadsheet (must have .xml)
characterTag = 'jorsaw' -- Set this to what the sprite should be named.

noteNames = {
    'Jorsawsee Note', -- Change this to whatever your note is named in path "custom_notetypes".
...
function onCreatePost()
    setProperty('jorsaw.alpha', 0);
end


function onSongStart()
doTweenAlpha('sdsfffd', 'jorsaw', 1, 1, 'linear')
end
function onStepHit()
if curStep == 3391 then
doTweenAlpha('whddyfdf', 'jorsaw', 0, 0.1, 'linear')
end
if curStep == 3920 then
doTweenAlpha('ddige', 'jorsaw', 0, 0.1, 'linear')
end
if curStep == 1431 then
doTweenAlpha('dfhdfdd', 'jorsaw', 0, 1, 'linear')
end
if curStep == 1685 then
doTweenAlpha('gddfghfdgsd', 'jorsaw', 1, 0.1, 'linear')
end
end
...
function onSpawnNote(membersIndex, noteData, noteType, isSustainNote)
	...
	if noteType == 'Jorsawsee Note' then
...
function goodNoteHit(membersIndex, noteData, noteType, isSustainNote) 
    ...
    elseif noteType == noteNames[21] and not isDanceIdle then
        animSuffix = '-alt'
        if getProperty(characterName .. '.animation.curAnim.name') == singAnimations[1] .. animSuffix then
            danced = true
        elseif getProperty(characterName .. '.animation.curAnim.name') == singAnimations[2] .. animSuffix then
