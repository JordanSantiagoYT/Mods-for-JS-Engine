characterPath = 'characters/maroon' -- Picture File name of spreadsheet (must have .xml)
characterTag = 'playerskiller' -- Set this to what the sprite should be named.

noteNames = {
    'Maroon Note', -- Change this to whatever your note is named in path "custom_notetypes".
...
objectOrder = 4 -- Normally, values over 7 go over GF (7), Dad (8) and BF (9). (Default: 9)
...
characterOffsets = {
    ...
    224, -200, -- Down
    355, -186, -- Up
    139, -217, -- Right -- i have to do this AGAIN so that when gray hits an all opps note he doesnt slide all over the place
...
function onCreatePost()
    setProperty('playerskiller.alpha', 0);
end


function onSongStart()
doTweenAlpha('grasdffsdy', 'playerskiller', 1, 1, 'linear')
end
function onStepHit()
if curStep == 3391 then
doTweenAlpha('grsdfa', 'playerskiller', 0, 0.1, 'linear')
end
if curStep == 1431 then
doTweenAlpha('grsdf', 'playerskiller', 0, 1, 'linear')
end
if curStep == 1685 then
doTweenAlpha('gsdf', 'playerskiller', 1, 0.1, 'linear')
end
end
...
function opponentNoteHit(membersIndex, noteData, noteType, isSustainNote) 
    ...
    if noteType == noteNames[21] then
        animSuffix = '-alt'
        playAnim(characterName, singAnimations[noteData + 1] .. animSuffix, true);
        holdTimer = 0
    
    elseif noteType == noteNames[12] and not isDanceIdle then
        animSuffix = '-alt'
        if getProperty(characterName .. '.animation.curAnim.name') == singAnimations[1] .. animSuffix then
            danced = true
        elseif getProperty(characterName .. '.animation.curAnim.name') == singAnimations[2] .. animSuffix then
