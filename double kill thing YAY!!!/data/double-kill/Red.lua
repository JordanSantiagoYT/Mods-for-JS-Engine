characterPath = 'characters/impostor' -- Picture File name of spreadsheet (must have .xml)
characterTag = 'whentheimpostorissus' -- Set this to what the sprite should be named.

noteNames = {
    'Red Note', -- Change this to whatever your note is named in path "custom_notetypes".
...
objectOrder = 8 -- Normally, values over 7 go over GF (7), Dad (8) and BF (9). (Default: 9)
...
animPrefixes = {
    ...
    'impostor down', -- Down
    'impostor up', -- Up
    'impostor right', -- Right

    'imposter left', -- Left (ALT)
    'impostor down', -- Down (ALT)
    'impostor up', -- Up (ALT)
    'impostor right', -- Right (ALT)
    ...
    'impostor idle', -- Dance LEFT (Indices if needed.)
    'impostor idle' -- Dance RIGHT (Indices if needed.)
}
...
characterOffsets = {
    ...
    40, -235, -- Down
    -16, -234, -- Up
    -73, -294, -- Right -- i have to do this AGAIN so that when red hits an all opps note he doesnt slide all over the place
...
function onCreatePost()
    setProperty('whentheimpostorissus.alpha', 0);
end


function onSongStart()
doTweenAlpha('f', 'whentheimpostorissus', 1, 1, 'linear')
end
function onStepHit()
if curStep == 3391 then
doTweenAlpha('dfdf', 'whentheimpostorissus', 0, 0.1, 'linear')
end
if curStep == 3920 then
doTweenAlpha('die', 'whentheimpostorissus', 0, 0.1, 'linear')
end
if curStep == 1431 then
doTweenAlpha('dffdd', 'whentheimpostorissus', 0, 1, 'linear')
end
if curStep == 1685 then
doTweenAlpha('gdfgfdgsd', 'whentheimpostorissus', 1, 0.1, 'linear')
end
end
