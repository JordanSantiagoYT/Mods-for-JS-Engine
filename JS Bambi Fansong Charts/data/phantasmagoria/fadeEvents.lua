function onCreatePost()
	makeLuaSprite('flash', '', 0, 0);
	makeGraphic('flash', 1280, 720, '000000')
	addLuaSprite('flash', true);
	setLuaSpriteScrollFactor('flash', 0, 0)
	setProperty('flash.scale.x', 2)
	setProperty('flash.scale.y', 2)
	setProperty('flash.alpha', 1)
	setObjectCamera('flash', 'camOther')

	makeLuaText('Credits', "Phantasmagoria\n\nSong by joolieiscool\n\nChart by Jordan Santiago", 1250, 0, 720)
	setTextAlignment('Credits', 'Center')
	addLuaText('Credits')
	setTextSize('Credits', 40)
	setObjectCamera('Credits', 'camOther')
	setTextBorder('Credits', 3, '000000')
	setProperty('ytWatermark.alpha', 0)
	setProperty('boyfriend.singDuration', 16)
	setProperty('dad.singDuration', 16)
end

function onSongStart()
	doTweenAlpha('fadeIntro', 'flash', 0, (240/curBpm) * 16, 'linear')
end
function onStepHit()
	if curStep == 256 then
		doTweenY('creditTween', 'Credits', 250, (720/curBpm), 'expoOut')
	end
	if curStep == 352 then
		doTweenY('creditTween', 'Credits', -420, (720/curBpm), 'expoIn')
		doTweenAlpha('ytWatermarkIn', 'ytWatermark', 0.5, (720/curBpm), 'linear')
	end
	if curStep == 2816 then
		cameraFlash('camOther', 'ffffff', 0.7)
		setProperty('flash.alpha', 1)
	end
	if curStep == 3072 then
		doTweenAlpha('FadeIn2', 'flash', 0, (240/curBpm) * 16, 'linear')
	end
	if curStep == 5568 then
		doTweenAlpha('fadeOut', 'flash', 1, (240/curBpm) * 4, 'linear')
	end
end

function onEvent(n,v1,v2)
	if n == 'Change Character' then
		setProperty('boyfriend.singDuration', 16)
	end
end