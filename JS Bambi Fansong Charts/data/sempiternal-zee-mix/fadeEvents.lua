function onCreatePost()
	makeLuaSprite('flash', '', 0, 0);
	makeGraphic('flash', 1280, 720, '000000')
	addLuaSprite('flash', true);
	setLuaSpriteScrollFactor('flash', 0, 0)
	setProperty('flash.scale.x', 2)
	setProperty('flash.scale.y', 2)
	setProperty('flash.alpha', 1)
	setObjectCamera('flash', 'camGame')

	makeLuaText('Credits', "Sempiternal (Zee Mix)\n\nSong by ChaoZBusterZ\n\nChart by Jordan Santiago\n\nOpponent Sprites by Enzo Polis", 1250, 0, 740)
	setTextAlignment('Credits', 'Center')
	addLuaText('Credits')
	setTextSize('Credits', 40)
	setObjectCamera('Credits', 'camOther')
	setTextBorder('Credits', 3, '000000')

	setProperty('camZoomingDecay', 2.5)
	setProperty('ytWatermark.alpha', 0)
end

function onSongStart()
	doTweenAlpha('fadeIntro', 'flash', 0, (240/curBpm) * 8, 'linear')
end
function onStepHit()
	if curStep == 512 then
		doTweenY('creditTween', 'Credits', 240, (480/curBpm), 'expoOut')
	end
	if curStep == 640 then
		doTweenY('creditTween', 'Credits', -480, (480/curBpm), 'expoIn')
		doTweenAlpha('ytWatermarkIn', 'ytWatermark', 0.5, (480/curBpm), 'linear')
	end
	if (curStep > 1024 and curStep < 2048) or (curStep > 3584 and curStep < 4096) then
		if curStep % 256 == 136 then
			setProperty('flash.alpha', 1)
		elseif curStep % 256 == 144 then
			setProperty('flash.alpha', 0)
			cameraFlash('camGame', 'ffffff', 0.7)
		end
	end
	if curStep == 4352 then
		setObjectCamera('flash', 'camOther')
		doTweenAlpha('fadeOut', 'flash', 1, (240/curBpm) * 8, 'linear')
	end
end