...
void mainImage( out vec4 fragColor, vec2 fragCoord ) {
    ...
    col = mix(col, pow(col.x*vec3(0, 0, 0), vec3(.5, 0, 1)), 
              dot(sin(rd.yzx*0. + sin(rd.zxy*0.)), vec3(.0)) + .0);
    ...
    col = mix(col, vec3(col.x*col.x*.4, 0, col.x*col.x*.9), 
             dot(sin(rd.yzx*4. + sin(rd.zxy*9.)), vec3(.0)) + .7);
