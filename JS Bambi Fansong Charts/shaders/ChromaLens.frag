...
// original effect is from here: https://shadertoy.com/view/4ttXWM

#pragma header

#define round(a) floor(a + 1)
...
color = clamp(openfl_ColorOffsetv + (color * colorMultiplier), 0.0, 2.0);
if (color.a > 0.0)
{
...
uniform float distortion = 0.2;// the bias of the barrel distortion
const float iterations = 10.0;// how many samples to use for edge blur
uniform float strength = 0.0065;// how much edge blur is applied (to obscure the r, g, b separation)
uniform float separation = 0.3;// how much to separate the r, g and b
