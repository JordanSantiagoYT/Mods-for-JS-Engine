...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	vec3 objCol = vec3(f*f, pow(f, 0.)*.0, f*f*.0);
    // Blood... ruby red.
    //vec3 objCol = vec3(f*f, pow(f, 16.), pow(f, 8.)*.5);
    

    // Using the values above to produce the final color.   
    col = (objCol*(diff + .5) + vec3(.3, .0, 0)*spec*1.5)*atten;


    // Done. 
	fragColor = vec4(sqrt(min(col, 1.)), texture(iChannel0, fragCoord / iResolution.xy).a);
}
